...
namespace TestTask
{
    public static class LoggerService
    {
        ...
        static LoggerService()
        {
            ...
            var writer = new Lazy<IWriter>(() => new WriterPool(new List<IWriter> { new ConsoleWriter(), new FileLogWriter(fileName) }));
            var batchWriter = new Lazy<IBatchWriter>(() => new BatchWriterPool(new List<IBatchWriter> { new BatchConsoleWriter(), new BatchFileLogWriter(fileName) }));
